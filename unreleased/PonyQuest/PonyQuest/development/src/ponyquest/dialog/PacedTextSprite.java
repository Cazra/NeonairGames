...
public class PacedTextSprite extends TextSprite {
  ...
  /** Returns true iff the entire text for the current paragraph is displayed. */
  public boolean isParaDone() {
    return (curLength == text.length());
  }
  
  
  /** Returns true iff there is another paragraph after the current one. */
  public boolean hasNextPara() {
    return (paraIndex < paragraphs.size() - 1);
  }
