...
public class XMLParser {
  ...
  /** Parses an XML Document from a resource. If it is successful, the element node for the document is returned. Otherwise it returns null. */
  public static Element parse(String f) {
    try {
      if(!f.startsWith("/")) {
        f = "/" + f;
      }
      InputStream is = XMLParser.class.getResourceAsStream(f);
      DocumentBuilder docBuilder = getDocumentBuilder();
  ...
  public static void main(String[] args) {
    ...
    Element doc = XMLParser.parse(path); // new File(path));
